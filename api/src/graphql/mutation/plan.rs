...
#[Object]
impl PlanMutation {
    pub async fn create_plan(&self, ctx: &Context<'_>, file: Upload) -> Result<plan::PlanNode> {
        let db = ctx.data::<Database>().unwrap();
        let conn = db.get_connection();
        let converted_file: Result<Vec<_>, _> = file.value(ctx).unwrap().content.bytes().collect();
